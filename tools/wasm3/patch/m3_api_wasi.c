...
static inline
const struct __clockid * convert_clockid(__wasi_clockid_t in) {
    switch (in) {
    ...
    default: return (const struct __clockid *)-1;
    }
}
...
m3ApiRawFunction(m3_wasi_generic_clock_res_get)
{
    ...
    const struct __clockid * clk = convert_clockid(wasi_clk_id);
    if ((int)clk < 0) m3ApiReturn(__WASI_ERRNO_INVAL);
